test_that("single gene complete integration gives correct results - Test1", {
  ...
  y_gene_offset <- lapply(1:nrow(counts), function(x) matrix(1, 1, ncol(counts)))
  design_single <- list()
...
y_gene_offset <- lapply(1:nrow(mirna_exp_model), function(x) matrix(1, 1, ncol(mirna_exp_model)))
