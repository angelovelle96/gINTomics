...
tmp2 <- tmp2[names(tmp),]
rownames(tmp2) <- tmp
tmp2$miRBase_id <- rownames(tmp2)
rownames(tmp2) <- NULL

genes_annotation$hsa$miRBase_id <- NA
...
genes_annotation$hsa$original_hgnc_symbol <- genes_annotation$hsa$hgnc_symbol
genes_annotation$hsa$hgnc_symbol[!is.na(genes_annotation$hsa$miRBase_id)] <- 
  genes_annotation$hsa$miRBase_id[!is.na(genes_annotation$hsa$miRBase_id)]
