  ...
  if(plotType == "histo"){
    handler <- downloadHandler(
      ...
      content = function(file){
        if(tf){
        reactive_histo_table <- .prepare_reactive_histoTF_table(deg=deg,
                                                              data_table=data_table,
                                                              input=input,
                                                              output=output)
        }else{
          reactive_histo_table <- .prepare_reactive_histo_table(deg=deg,
                                                                data_table=data_table,
                                                                input=input,
                                                                output=output)
...
run_shiny <- function(multiomics_integration){
  ...
  server <- function(input, output, session) {
    ...
    output$download_csv_ridge_deg <- download_csv(type = "all",
                                                  ...
                                                  data_table=data_table)
    ## ----------------------- HISTO SERVERS --------------------------
    callModule(.server_histo_gen,
               id = "histo_gen",
               data_table = data_table)
    callModule(.server_histo_trans,
               id = "histo_trans",
               data_table = data_table)
    callModule(.server_histo_deg,
               id = "histo_deg",
               data_table = data_table)
